/* enhance.css - Luliy Blog v9
   Sections:
   00  Font imports
   01  CSS Variables + Reset
   02  Homepage Hero (first-visit full-screen, animation sequence)
   03  Background image
   04  Typography — fonts / day=black / night=white / sans toggle
   05  Navbar — liquid glass + collapsible + homepage button
   06  Hero cluster
   07  Hero banner
   08  Progress bar
   09  Cards (home grid + shine hover + year grouping)
   10  Date badge (relative time)
   11  Pinned section (multi-level)
   12  Article reading panels (theme-specific)
   13  Code blocks (day + dark + line numbers)
   14  Tables (day + dark + sticky header)
   15  Article theme CSS variables (6 themes)
   16  TOC scroll-spy + active highlight (theme-aware)
   ...
   22  Uptime
   23  Timeline / archive
   24  Homepage bottom gallery banner (grid + custom)
...
/* 专注模式（Focus Mode）编辑部风排版专用字体 —— 仅在 .luliy-focus-mode 下引用，
   衬线正文用 Source Serif 4（英文/数字），中文回退到 Noto Serif SC。 */
@import url('https://fonts.googleapis.com/css2?family=Source+Serif+4:opsz,wght@8..60,400;8..60,600&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&family=Noto+Serif+SC:wght@400;600&display=swap');
...
/* Guard: 100vw elements (hero) must never cause horizontal scroll.
   IMPORTANT: clip on <html> ONLY — Gmeek's <body> is a centred
   max-width container; clipping it would crop the full-bleed hero. */
...
/* ══════════════════════════════════════════════════════════
   02  Homepage Hero — full-screen, scroll to enter
══════════════════════════════════════════════════════════ */
#luliy-hero {
  /* Escape any parent padding/margin to be truly full-viewport */
  position: relative;
  animation: luliyHeroBgIn 1.8s ease both;
  width: 100vw;
  max-width: none !important;   /* defeat any inherited max-width */
  ...
  z-index: 1;
  margin-bottom: 8px;
  /* ★ 大改后：整张图都可点击进入主页，不再依赖滚动 */
  cursor: pointer;
  transition: opacity 0.55s ease;
}
/* 点击后的淡出（JS 加 is-leaving，淡出完再从 DOM 移除） */
#luliy-hero.is-leaving { opacity: 0; pointer-events: none; }
#luliy-hero-inner {
  position: relative;
  z-index: 2;
...
#luliy-hero-title {
  ...
  text-shadow: 0 4px 40px rgba(0,0,0,0.5), 0 2px 8px rgba(0,0,0,0.4);
  animation: luliyHeroTitleIn 1.6s cubic-bezier(.2,.7,.3,1) both;
}
...
#luliy-hero-subtitle {
  ...
  animation: luliyHeroIn 1.3s cubic-bezier(.2,.7,.3,1) 0.25s both;
}
#luliy-hero-hint {
  position: absolute;
  bottom: 36px; left: 50%;
  ...
  animation: luliyHeroBounce 2s ease-in-out infinite, luliyHeroIn 1.3s ease 0.6s both;
  transition: background 0.2s;
}
#luliy-hero-hint:hover { background: rgba(255,255,255,0.24); }
#luliy-hero-hint:focus-visible {
...
/* Reduce-motion: hero appears instantly, no stagger */
body.luliy-reduce-motion #luliy-hero,
body.luliy-reduce-motion #luliy-hero-title,
body.luliy-reduce-motion #luliy-hero-subtitle,
body.luliy-reduce-motion #luliy-hero-hint {
  animation: none !important;
  opacity: 1 !important; transform: none !important; filter: none !important;
}
#luliy-hero-hint { transform: translateX(-50%); }  /* keep centring when reduce overrides transform */
body.luliy-reduce-motion #luliy-hero-hint { transform: translateX(-50%) !important; }
...
body.luliy-reduce-motion #luliy-hero-hint { animation: none; }
/* On article / single pages there is no hero, so this never applies. */

/* ── Hero corner badge (Rockstar "Pre-order on ..." style tag) ── */
#luliy-hero-badge {
  position: absolute;
  top: 28px; left: 28px;
  z-index: 3;
  ...
  animation: luliyHeroIn 1.2s cubic-bezier(.2,.7,.3,1) 0.15s both;
}
@media (max-width: 600px) {
  #luliy-hero-badge { top: 16px; left: 16px; font-size: 12px; padding: 6px 12px; }
}
body.luliy-reduce-motion #luliy-hero-badge { animation: none !important; opacity: 1 !important; }
...
.luliy-card-title, #luliy-hero-name,
.blogTitle, .blogTitle h1, .blogTitle a {
...
body.luliy-canger #postBody blockquote {
  font-family: 'Ma Shan Zheng', 'LXGW WenKai Screen', serif !important;
}
...
/* ── Base #header — hosts the floating hero card ─────────── */
#header {
...
/* ════════ Hero card — frosted glass, theme-aware ════════ */
#luliy-nav-rebuilt {
  width: 100%;
  max-width: 1480px;
  display: grid;
  ...
  position: relative;
  /* Grey frosted glass */
  background: var(--th-hero-bg, rgba(245,245,250,0.62));
  -webkit-backdrop-filter: blur(28px) saturate(1.7);
  backdrop-filter: blur(28px) saturate(1.7);
  border: 1px solid var(--th-hero-border, rgba(255,255,255,0.55));
  box-shadow: 0 10px 40px rgba(0,0,0,0.12), 0 1px 0 rgba(255,255,255,0.6) inset;
  transition: opacity 0.4s ease;
}
[data-color-mode="dark"] #luliy-nav-rebuilt {
  background: var(--th-hero-bg, rgba(18,16,26,0.66));
  border-color: var(--th-hero-border, rgba(255,255,255,0.10));
  box-shadow: 0 10px 40px rgba(0,0,0,0.5), 0 1px 0 rgba(255,255,255,0.06) inset;
}
/* Theme accent top border on the card */
body[data-luliy-theme="default"] #luliy-nav-rebuilt  { border-top: 2px solid rgba(130,80,223,0.55); }
body[data-luliy-theme="sakura"] #luliy-nav-rebuilt   { border-top: 2px solid rgba(201,64,112,0.55); }
body[data-luliy-theme="your-name"] #luliy-nav-rebuilt{ border-top: 2px solid rgba(255,169,77,0.65); }
body[data-luliy-theme="space"] #luliy-nav-rebuilt    { border-top: 2px solid rgba(0,229,255,0.55); }
body[data-luliy-theme="cyberpunk"] #luliy-nav-rebuilt    { border-top: 2px solid rgba(255,43,214,0.55); }
body[data-luliy-theme="sunset"] #luliy-nav-rebuilt   { border-top: 2px solid rgba(232,168,56,0.65); }
body[data-luliy-theme="mono"] #luliy-nav-rebuilt     { border-top: 2px solid rgba(150,150,150,0.5); }
...
#luliy-nav-avatar-link { display: flex; align-items: center; flex-shrink: 0; text-decoration: none; }
#luliy-nav-avatar {
  width: 60px; height: 60px; border-radius: 50%;
  object-fit: cover;
  border: 3px solid var(--th-hero-avatar-ring, rgba(255,255,255,0.85));
  box-shadow: 0 4px 16px rgba(0,0,0,0.18);
  transition: border-color 0.2s, transform 0.2s;
}
#luliy-nav-avatar:hover { border-color: var(--th-accent, #8250df); transform: scale(1.05); }
#luliy-nav-blogname {
...
#luliy-hero-divider {
  ...
  background: var(--th-hero-divider, rgba(0,0,0,0.14));
  flex-shrink: 0;
}
[data-color-mode="dark"] #luliy-hero-divider { background: rgba(255,255,255,0.16); }
#luliy-hero-timeblock { display: flex; flex-direction: column; gap: 3px; }
#luliy-hero-time {
  display: flex; align-items: center; gap: 6px;
  font-size: 22px; font-weight: 700;
  font-family: 'LXGW WenKai Mono', 'Noto Sans SC', monospace;
  color: var(--th-accent, #8250df);
  line-height: 1;
}
#luliy-hero-clock { display: inline-flex; color: var(--th-accent, #8250df); }
#luliy-hero-date {
  font-size: 12px; letter-spacing: 0.5px;
  color: var(--th-hero-muted, rgba(0,0,0,0.45));
  font-family: 'LXGW WenKai Mono', monospace;
}
[data-color-mode="dark"] #luliy-hero-date { color: rgba(255,255,255,0.5); }

/* Day/night circle — beside the time block */
#luliy-nav-circle, .luliy-hero-circle {
  width: 34px; height: 34px; border-radius: 50%;
  display: inline-flex !important; align-items: center; justify-content: center;
  flex-shrink: 0;
  background: var(--th-accent-soft, rgba(130,80,223,0.12)) !important;
  color: var(--th-accent, #8250df) !important; cursor: pointer;
  transition: background 0.18s, transform 0.18s;
}
#luliy-nav-circle:hover { background: var(--th-accent-hover, rgba(130,80,223,0.24)) !important; transform: scale(1.1); }
#luliy-nav-circle svg, .luliy-hero-circle svg { fill: currentColor !important; width: 17px; height: 17px; }

/* ── CENTRE-BOTTOM: link capsule bar — full-width row 2 ─── */
#luliy-hero-capsule {
  grid-area: capsule;
  display: flex; align-items: center;
  /* ★ bug修复：原 justify-content:center + overflow-x:auto 组合是经典
     CSS 陷阱——内容溢出时居中会让左侧溢出量变成「负向 scrollLeft」，
     浏览器无法滚到那里，造成左侧文字永久裁切不可见（如"Favorites"
     显示成"rites"），右侧也只能看到居中窗口内的一段。
     现在改用 flex-start + overflow:hidden + 纯CSS响应式降级（窄屏只显
     示图标，见 section 38），不再用 JS 测量溢出，没有滚动、没有
     "测量时机不对"的问题，这个陷阱不会再出现。 */
  justify-content: flex-start;
  gap: 0;
  padding: 6px 8px;
  border-radius: 14px;
  background: var(--th-hero-capsule, rgba(255,255,255,0.45));
  border: 1px solid var(--th-hero-border, rgba(255,255,255,0.5));
  -webkit-backdrop-filter: blur(12px); backdrop-filter: blur(12px);
  width: 100%;
  min-width: 0;
  overflow: hidden;
}
[data-color-mode="dark"] #luliy-hero-capsule {
...
.luliy-hero-cap-link {
  ...
  color: var(--th-hero-fg, #2a2440) !important;
...
[data-color-mode="dark"] .luliy-hero-cap-link { color: var(--th-hero-fg, rgba(245,242,255,0.92)) !important; }
.luliy-hero-cap-link:hover {
  background: var(--th-accent-soft, rgba(130,80,223,0.14)) !important;
  color: var(--th-accent, #8250df) !important;
  transform: translateY(-1px);
}
[data-color-mode="dark"] .luliy-hero-cap-link:hover { color: #fff !important; }
.luliy-hero-cap-link svg { width: 17px; height: 17px; }
.luliy-hero-cap-txt { text-transform: capitalize; }
.luliy-hero-cap-sep {
  width: 1px; height: 18px; flex-shrink: 0;
  background: var(--th-hero-divider, rgba(0,0,0,0.1));
}
[data-color-mode="dark"] .luliy-hero-cap-sep { background: rgba(255,255,255,0.12); }

/* ── RIGHT: settings slot (the toolbar pill relocates here) ─ */
#luliy-hero-right {
  grid-area: right;
  display: flex; align-items: center; justify-content: flex-end;
  min-width: 56px; align-self: center;
}

/* When the pill lives in the hero, neutralise its fixed positioning */
#luliy-hero-right #luliy-toolbar {
  position: relative !important;
  top: auto !important; right: auto !important; bottom: auto !important; left: auto !important;
}
/* Panel drops down-left from the pill when it lives in the hero */
#luliy-hero-right #luliy-ctrl-panel {
...
#luliy-hero-subtitle {
  ...
  background: var(--th-accent-soft, rgba(130,80,223,0.10));
  border: 1px solid var(--th-accent-soft, rgba(130,80,223,0.20));
  transition: opacity 0.2s, background 0.2s, transform 0.2s;
  white-space: nowrap; flex-shrink: 0;
  -webkit-user-select: none; user-select: none;
}
#luliy-hero-subtitle:hover { opacity: 1; transform: scale(1.05); background: var(--th-accent-soft, rgba(130,80,223,0.18)); }
[data-color-mode="dark"] #luliy-hero-subtitle { color: var(--th-accent, #cba6f7); }
...
/* ══════════════════════════════════════════════════════════
   07  Hero banner (scroll-fold on homepage)
══════════════════════════════════════════════════════════ */
#luliy-hero-banner {
  ...
  border: 1px solid var(--th-accent-soft, rgba(130,80,223,0.30));
  border-radius: 14px;
  text-align: center;
  font-size: 14px; font-weight: 800; letter-spacing: 2px;
  color: var(--th-accent, #8250df);
  text-shadow: 0 1px 2px rgba(255,255,255,0.5);
  box-shadow: 0 4px 20px var(--th-accent-soft, rgba(130,80,223,0.12));
  box-sizing: border-box;
  will-change: transform, opacity;
...
[data-color-mode="dark"] #luliy-hero-banner {
  ...
  border-color: var(--th-accent-soft, rgba(130,80,223,0.40));
...
══════════════════════════════════════════════════════════ */
@media (max-width: 768px) {
  /* ══ 仅保留：触摸优化 + 表格横滑 + 内容排版（旧 hero/汉堡/mobrow 已删，导航改用抽屉）══ */
...
@media (max-width: 768px) {
  ...
  /* (3) 文章容器：移动端去掉常驻的 will-change，避免长页面把整篇文章
     永久提升为合成层、白白占用大量显存。 */
  #luliy-hero-banner { will-change: auto !important; }
}
...
/* ══════════════════════════════════════════════════════════
   32  导航栏透明度开关按钮样式  ★新增
══════════════════════════════════════════════════════════ */
.luliy-fade-toggle-row {
  width: 100%; display: flex; align-items: center;
  padding: 9px 14px; gap: 8px; border: none;
...
.luliy-fade-toggle-row:hover { background: var(--th-accent-soft, rgba(130,80,223,0.10)); }
.luliy-fade-toggle-row.is-active .luliy-fade-badge { color: #22c55e; }
.luliy-fade-toggle-row:not(.is-active) .luliy-fade-badge { color: #f87171; }
.luliy-fade-badge {
...
/* ══════════════════════════════════════════════════════════
   35  左滑抽屉导航 + ☰ 按钮 + 双导航模式  ★全新
══════════════════════════════════════════════════════════ */

/* ── 双导航模式显隐 ──
   body.luliy-nav-hero   → 显示桌面 hero 导航栏，隐藏 ☰
   body.luliy-nav-drawer → 隐藏 hero 导航栏，显示 ☰ */
body.luliy-nav-drawer #luliy-nav-rebuilt { display: none !important; }
body.luliy-nav-drawer #header { min-height: 0 !important; padding: 0 !important; background: transparent !important; }
body.luliy-nav-hero   #luliy-ham-btn { display: none !important; }
...
body.luliy-nav-drawer #luliy-hero-banner { margin-top: 70px; }
...
body.luliy-drawer-open #luliy-aplayer { z-index: 8000; }

/* ══════════════════════════════════════════════════════════
   36  桌面端 hero 导航栏响应式补强  ★
   需求：桌面端任何窗口尺寸下，hero 元素可随窗口缩放，但绝不被
   遮挡/隐藏/折叠/裁剪。用 min-width:0 + flex 收缩 + 文字省略实现。
══════════════════════════════════════════════════════════ */
#luliy-nav-rebuilt { overflow: visible; }   /* 不裁剪子元素 */

/* 左区(头像+名+时间)允许整体收缩，名字过长省略而非溢出 */
#luliy-hero-left { min-width: 0; }
#luliy-nav-blogname {
...
#luliy-hero-subtitle { min-width: 0; }

/* 链接胶囊：min-width:0 + overflow:hidden 已在基础规则定义，
   窄屏降级为纯图标见 section 38，不再有 JS 测量/滚动相关的设置 */

/* 右区(设置pill)不收缩、始终可见 */
#luliy-hero-right { flex-shrink: 0; }

/* 中窄桌面(769~1024)：缩小间距与字号，保持单行不折叠 */
@media (min-width: 769px) and (max-width: 1024px) {
  #luliy-nav-rebuilt { padding: 12px 18px 10px; gap: 10px 14px; }
  #luliy-nav-blogname { font-size: 20px; max-width: 24vw; }
  #luliy-hero-time { font-size: 15px; }
  #luliy-hero-subtitle { font-size: 12px; padding: 4px 10px; }
  .luliy-hero-cap-link { padding: 5px 10px; font-size: 13px; }
}
...
/* ── 抽屉顶部：切换按钮 + 日夜按钮 并排 ── */
.ldh-btn-row { display: flex; gap: 8px; margin-top: 14px; align-items: stretch; }
#luliy-drawer-dnbtn { margin-top: 0 !important; flex: 1; }   /* 日夜按钮占满该行 */
#luliy-drawer-dnbtn {
...
/* ══════════════════════════════════════════════════════════
   38  hero 链接胶囊响应式降级（替代已删除的 More 下拉）  ★
   逻辑：More 机制依赖 JS 测量元素宽度来决定收纳，但测量时机很容易
   出错（隐藏态测量为 0、resize 不覆盖所有触发场景等），上一版就因
   此出现过显示异常。改用纯 CSS 方案后没有这些问题：
   - 空间还够时，图标+文字都显示
   - 视口变窄、capsule 撑不下全部链接文字时，文字自动隐藏，只留图标
   - 纯 CSS 渐进降级，没有"测量时机"这个概念，不存在出错的时间窗口
   断点选择：每个链接图标+间距约 36px，文字额外占 50~70px，
   1024px 临界是经验值——可按实际链接数量调整。 ══════════════════════════════════════════════════════════ */
@media (max-width: 1024px) {
  .luliy-hero-cap-link .luliy-hero-cap-txt { display: none; }
  .luliy-hero-cap-link { padding: 7px 9px; }   /* 纯图标时缩小左右留白，更紧凑 */
}
/* 更窄时（逼近抽屉切换的临界区）进一步收紧分隔符间距，挤出更多空间 */
@media (max-width: 860px) {
  .luliy-hero-cap-sep { margin: 0 2px; }
  #luliy-hero-capsule { gap: 0; padding: 6px 6px; }
}
...
/* 极简模式：取消上面那套自定义光标，用系统默认箭头/手型 */
html:has(body.luliy-minimal),
body.luliy-minimal,
body.luliy-minimal * {
  cursor: auto !important;
}
body.luliy-minimal a,
body.luliy-minimal button,
body.luliy-minimal [role="button"],
body.luliy-minimal [onclick] {
...
/* —— 右侧常驻导航：Home / Archives / Chronicle / Book / About —— */
#luliy-min-nav {
  position: fixed;
  top: 56px; left: 40px; right: auto;
  z-index: 65;
...
#luliy-min-nav .luliy-min-nav-sep { color: var(--min-muted); font-weight: 400; }
...
.luliy-min-home-nav a {
  color: var(--min-title);
  text-decoration: none;
  transition: opacity 0.15s;
}
.luliy-min-home-nav a:hover { opacity: 0.55; }
.luliy-min-home-nav .luliy-min-nav-sep { margin: 0 12px; color: var(--min-muted); }
@media (max-width: 640px) {
  .luliy-min-home-nav { top: 14px; left: 16px; font-size: 16px; }
}
...
body.luliy-min-article #postBody {
  max-width: 680px;          /* 正文主列偏右、不居中，给左侧目录让位 */
  margin: 0;
  margin-left: auto;
  font-family: var(--min-sans);
  font-size: 16px;
...
body.luliy-min-article #postBody h3 {
  font-family: var(--min-sans);
  font-weight: 600; font-size: 18px; color: var(--min-title);
  margin: 36px 0 12px;
}
/* 四级标题：半粗体 + 左侧 ▍ 装饰，体量与正文接近但用色块强调 */
...
body.luliy-min-article #postBody h4::before {
  content: '▍';
  position: absolute;
  left: -2px; top: 0;
  color: var(--min-muted);
...
#luliy-min-toc {
  position: fixed;
  top: 120px; left: max(40px, calc((100vw - 1100px) / 2 + 48px)); right: auto;
...
/* ════════════════════════════════════════════════════════════
   ...
   触发：文章页内双击正文，或按 F 键（Esc 退出）。
   编辑部 / 学术期刊排版规范：象牙白底 + 近黑正文 + 单一陶土色强调，
   衬线正文 + 无衬线 UI + 等宽大写元信息，居中 720px 主列，1.65 行高，
   无阴影 / 无渐变 / 无圆角，1px 浅灰硬边框，大量留白，无背景图/纹理。
   仅在 body.luliy-minimal.luliy-focus-mode 下生效，不影响其他状态。
   ════════════════════════════════════════════════════════════ */
body.luliy-minimal.luliy-focus-mode {
  --focus-bg:      #faf9f5;   /* 暖象牙白，禁止纯白 */
  --focus-text:    #141413;   /* 近黑正文 */
  --focus-accent:  #d97757;   /* 唯一强调色：陶土橙，仅用于链接/强调 */
  --focus-border:  #d1cfc5;   /* 1px 浅灰硬边框 */
  --focus-serif:   'Source Serif 4', 'Lora', 'Noto Serif SC', Georgia, 'Times New Roman', serif;
  --focus-sans:    'Inter', -apple-system, 'PingFang SC', 'Microsoft YaHei', sans-serif;
  --focus-mono:    'JetBrains Mono', 'SFMono-Regular', ui-monospace, 'Courier New', monospace;

  background: var(--focus-bg) !important;
  background-image: none !important;
}
/* 隐藏一切会分散注意力的界面：右侧导航、悬浮目录、系统切换按钮 */
body.luliy-minimal.luliy-focus-mode #luliy-min-nav,
body.luliy-minimal.luliy-focus-mode #luliy-min-toc,
body.luliy-minimal.luliy-focus-mode #luliy-system-toggle {
  display: none !important;
}
/* 扁平化：全局去阴影 / 去渐变 / 去圆角，只保留 1px 硬边框 */
body.luliy-minimal.luliy-focus-mode #content,
body.luliy-minimal.luliy-focus-mode #content *,
body.luliy-minimal.luliy-focus-mode #postBody,
body.luliy-minimal.luliy-focus-mode #postBody * {
  box-shadow: none !important;
  background-image: none !important;
...
body.luliy-minimal.luliy-focus-mode #postBody {
  max-width: 720px !important;
  margin: 0 !important;
  font-family: var(--focus-serif);
  font-size: 18px;
  line-height: 1.65;
  color: var(--focus-text);
}
body.luliy-minimal.luliy-focus-mode #postBody h1,
body.luliy-minimal.luliy-focus-mode #postBody h2,
body.luliy-minimal.luliy-focus-mode #postBody h3,
body.luliy-minimal.luliy-focus-mode #postBody h4 {
  font-family: var(--focus-serif);
  color: var(--focus-text);
  font-weight: 600;
  letter-spacing: 0;
...
body.luliy-minimal.luliy-focus-mode #postBody h2 {
  font-size: 24px; margin: 64px 0 20px;
}
body.luliy-minimal.luliy-focus-mode #postBody h3 {
  font-size: 19px; margin: 44px 0 14px;
}
body.luliy-minimal.luliy-focus-mode #postBody h4 {
  font-size: 17px; margin: 32px 0 12px; padding-left: 0;
}
body.luliy-minimal.luliy-focus-mode #postBody h4::before { content: none; }
body.luliy-minimal.luliy-focus-mode #postBody p {
  margin: 0 0 28px;
  color: var(--focus-text);
}
/* 唯一强调色：只用在链接上 */
body.luliy-minimal.luliy-focus-mode #postBody a {
  color: var(--focus-accent);
  text-decoration: underline;
  text-underline-offset: 3px;
}
body.luliy-minimal.luliy-focus-mode #postBody strong,
body.luliy-minimal.luliy-focus-mode #postBody b {
  color: var(--focus-text);
  font-weight: 600;
}
body.luliy-minimal.luliy-focus-mode #postBody em,
body.luliy-minimal.luliy-focus-mode #postBody i {
  color: var(--focus-accent);
  font-style: italic;
}
/* 图片：无圆角、无阴影，细边框代替 */
body.luliy-minimal.luliy-focus-mode #postBody img {
  max-width: 100%;
  border: 1px solid var(--focus-border);
}
/* 代码块：扁平、单色，等宽字体 */
body.luliy-minimal.luliy-focus-mode #postBody pre,
body.luliy-minimal.luliy-focus-mode #postBody code {
  font-family: var(--focus-mono);
  background: #f2f0e9;
  border: 1px solid var(--focus-border);
}
body.luliy-minimal.luliy-focus-mode #postBody blockquote {
...
/* 元信息（日期/分类）：等宽大写字体，宽字距，用作细节而非重点 */
body.luliy-minimal.luliy-focus-mode #postBody .luliy-readmeta,
body.luliy-minimal.luliy-focus-mode .luliy-readmeta {
  font-family: var(--focus-mono);
  text-transform: uppercase;
...
/* 退出提示（居中底部小字，2.2s 后自动淡出） */
#luliy-focus-hint {
  position: fixed;
  left: 50%; bottom: 32px;
  transform: translateX(-50%);
  z-index: 200;
  font-family: var(--focus-mono, 'JetBrains Mono', monospace);
  font-size: 11px;
  text-transform: uppercase;
  letter-spacing: 0.12em;
  color: #141413;
...
#luliy-focus-hint.is-out { opacity: 0; }
@media (max-width: 640px) {
  body.luliy-minimal.luliy-focus-mode #content { padding: 72px 20px 120px !important; }
  body.luliy-minimal.luliy-focus-mode #postBody h1 { font-size: 28px; }
}
...
@media (max-width: 720px) {
  #luliy-min-nav { left: 14px; top: 14px; gap: 6px; font-size: 11px; }
  body.luliy-minimal .luliy-archives,
  body.luliy-minimal .luliy-chronicle,
...
body.luliy-min-article #postBody h1 {
  ...
  -webkit-text-fill-color: #1c1c1e !important;
  border: none !important;
  border-bottom: 1px solid rgba(28,28,30,0.12) !important;
}
body.luliy-min-article #postBody p,
...
/* 分割线：极细线 + 居中 ◇ 微弱装饰 */
body.luliy-min-article #postBody hr {
  border: 0 !important;
  height: 1px !important;
  background: rgba(28,28,30,0.16) !important;
  margin: 56px 0 !important;
  position: relative !important;
  box-shadow: none !important;
}
body.luliy-min-article #postBody hr::after {
  content: '◇';
  position: absolute;
  left: 50%; top: 50%;
  transform: translate(-50%, -50%);
  background: rgb(230,225,212);
  padding: 0 14px;
  color: var(--min-muted);
  font-size: 12px;
  line-height: 1;
...
/* —— 2. 内层容器（文章面板/表格容器等）强制透明、去掉磨砂玻璃效果 ——
   ★ 同样套上 html:has(body.luliy-minimal) 外壳。原来这条选择器特异性
   跟"Default 50% iOS26 liquid glass background"那条主题面板规则刚好
   打平，谁赢全看文件里谁写在后面——这在多次复制粘贴、分段编辑之后
   非常容易被破坏（顺序一旦被打乱，面板的磨砂玻璃背景就会重新盖回来，
   正是截图里圆角卡片+背景图纹理的来源）。现在这条选择器比任何主题面板
   规则都多两层（html + :has()），优先级稳赢，不再依赖文件顺序。 */
html:has(body.luliy-minimal) body.luliy-minimal,
html:has(body.luliy-minimal) body.luliy-minimal #content,
html:has(body.luliy-minimal) body.luliy-minimal .markdown-body,
html:has(body.luliy-minimal) body.luliy-minimal #postBody,
html:has(body.luliy-minimal) body.luliy-minimal .SideNav,
html:has(body.luliy-minimal) body.luliy-minimal main,
html:has(body.luliy-minimal) body.luliy-minimal article {
  background: transparent !important;
  background-image: none !important;
  box-shadow: none !important;
...
/* body 本体保留底色（作为全屏背景，首页之外的页面会被下面的背景图规则再覆盖一次） */
html:has(body.luliy-minimal) body.luliy-minimal { background: rgb(230,225,212) !important; }
...
/* mac 三点装饰：不受上面"pre 内全部强制透明背景"规则影响，
   保留原本的红/黄/绿配色，否则三个点会变成看不见的黑底黑点 */
body.luliy-min-article #postBody pre .mac-strip {
  background: rgba(0,0,0,0.04) !important;
  border-bottom: 1px solid rgba(28,28,30,0.10) !important;
}
body.luliy-min-article #postBody pre .mac-btn {
  -webkit-text-fill-color: initial !important;
}
body.luliy-min-article #postBody pre .mac-btn-red    { background: #ff5f57 !important; }
body.luliy-min-article #postBody pre .mac-btn-yellow { background: #ffbd2e !important; }
body.luliy-min-article #postBody pre .mac-btn-green  { background: #28c840 !important; }
body.luliy-min-article #postBody pre .mac-lang {
  color: var(--min-muted) !important;
  -webkit-text-fill-color: var(--min-muted) !important;
}
...
/* ════════════════════════════════════════════════════════════
   高对比度收尾层 —— 提升所有主题日/夜模式下文章可读性
   核心：提高文章面板不透明度（默认 0.5 → 0.82），让文字坐在
   更实的底上；强化正文/标题/链接/代码/按钮对比度。
   放在文件最末尾，确保优先级最高。
   ════════════════════════════════════════════════════════════ */

/* 1. 文章面板不透明度：完全交由用户的"透明度"滑杆控制，
      这里不再强制覆盖（之前固定 0.86/0.88 导致滑杆调了也没用，
      面板永远是接近全实心的样子，挡住了背景）。 */

/* 2. 白天模式：正文深色、标题更黑，保证 7:1 以上对比 */
[data-color-mode="light"] #postBody,
[data-color-mode="light"] #postBody p,
[data-color-mode="light"] #postBody li,
[data-color-mode="light"] #postBody span,
[data-color-mode="light"] #postBody td,
[data-color-mode="light"] #postBody figcaption {
  color: #1a1a1f !important;
  -webkit-text-fill-color: #1a1a1f !important;
}
[data-color-mode="light"] #postBody h1,
[data-color-mode="light"] #postBody h2,
[data-color-mode="light"] #postBody h3,
[data-color-mode="light"] #postBody h4 {
  color: #0d0d12 !important;
  -webkit-text-fill-color: #0d0d12 !important;
}

/* 3. 夜间模式：正文足够亮（避免灰蒙蒙），标题更白 */
[data-color-mode="dark"] #postBody,
[data-color-mode="dark"] #postBody p,
[data-color-mode="dark"] #postBody li,
[data-color-mode="dark"] #postBody span,
...
[data-color-mode="dark"] #postBody h1,
[data-color-mode="dark"] #postBody h2,
[data-color-mode="dark"] #postBody h3,
[data-color-mode="dark"] #postBody h4 {
  color: #f5f3fb !important;
  -webkit-text-fill-color: #f5f3fb !important;
}

/* 4. 链接：日/夜都用高对比强调色 + 下划线，确保可识别 */
[data-color-mode="light"] #postBody a {
  color: #5b21b6 !important; -webkit-text-fill-color: #5b21b6 !important;
  text-decoration: underline !important; text-underline-offset: 2px !important;
}
[data-color-mode="dark"] #postBody a {
  color: #b9a3ff !important; -webkit-text-fill-color: #b9a3ff !important;
  text-decoration: underline !important; text-underline-offset: 2px !important;
}

/* 5. 行内代码：日/夜都有清晰底色和高对比文字 */
[data-color-mode="light"] #postBody code {
  background: rgba(28,28,34,0.07) !important; color: #9333ea !important;
  -webkit-text-fill-color: #9333ea !important; padding: 2px 6px; border-radius: 4px;
}
[data-color-mode="dark"] #postBody code {
  background: rgba(255,255,255,0.10) !important; color: #d6bcfa !important;
  -webkit-text-fill-color: #d6bcfa !important; padding: 2px 6px; border-radius: 4px;
}

/* 6. 功能按钮（浮动面板/抽屉/FAB）高对比 */
[data-color-mode="light"] #luliy-ham-btn span { background: #1a1a1f !important; }
[data-color-mode="dark"] #luliy-ham-btn span { background: #f0eef8 !important; }
[data-color-mode="light"] #luliy-issues-link,
[data-color-mode="light"] #luliy-to-minimal,
[data-color-mode="light"] #luliy-top-link { color: #2a2440 !important; }
[data-color-mode="dark"] #luliy-issues-link,
[data-color-mode="dark"] #luliy-to-minimal,
[data-color-mode="dark"] #luliy-top-link { color: #e8e6f0 !important; }

/* 7. 文末字数信息条：日/夜都清晰可读 */
[data-color-mode="light"] #luliy-post-footer-bar { color: #555 !important; border-top-color: rgba(0,0,0,0.10) !important; }
[data-color-mode="dark"] #luliy-post-footer-bar { color: #aaa !important; border-top-color: rgba(255,255,255,0.12) !important; }

/* ════════ 极简模式：隐藏右下角 目录/搜索/回顶 三个悬浮按钮 ════════ */
body.luliy-minimal #luliy-toc-fab,
body.luliy-minimal #luliy-search-fab,
body.luliy-minimal #luliy-search-bar,
body.luliy-minimal #luliy-back-top {
  display: none !important;
}

/* ════════ 极简模式：文章表格去掉所有样式，仅保留细黑线边框 ════════ */
html body.luliy-minimal #postBody table {
  width: 100% !important;
...
html body.luliy-minimal #postBody thead tr,
html body.luliy-minimal #postBody tr,
html body.luliy-minimal #postBody th,
html body.luliy-minimal #postBody td {
  background: transparent !important;
  background-image: none !important;
  box-shadow: none !important;
  border-radius: 0 !important;
...
html body.luliy-minimal #postBody th { font-weight: 700 !important; }

/* ════════ 极简模式：右侧目录跟随页面滚动，当前项用细竖线高亮 ════════ */
#luliy-min-toc a {
...
#luliy-min-toc a.is-current {
  color: var(--min-title) !important;
  -webkit-text-fill-color: var(--min-title) !important;
  font-weight: 600;
  border-left: 1px solid var(--min-title);
}
[data-color-mode] body.luliy-minimal #luliy-min-toc a.is-current {
  color: #1c1c1e !important;
  -webkit-text-fill-color: #1c1c1e !important;
  border-left: 1px solid #1c1c1e !important;
}

/* ════════ 极简模式：除首页外，加入统一背景图（叠加半透明遮罩保证可读性） ════════
   ★ 图片画在 html 上而不是 body 上 —— body 的实际渲染宽度可能比视口窄
   （内容区域居中、两侧留白），如果背景图画在 body 上就只会铺满 body 那一条，
   两侧露出 html 的纯色底，正是"背景图只在中间"这个问题的根源。
   html 才是真正贴满整个视口边缘的元素，背景图必须画在它上面。
   ★ 同时把 body 在这个场景下改成完全透明，否则 body 自己的纯色底会盖在
   html 的背景图上面，把图整个挡住。
   ★ 选择器继续套 html:has(body.luliy-minimal) 外壳并多加一层 :not()，
   保证优先级压得过前面"v2 总清洗"那条 background-image:none!important。 */
html:has(body.luliy-minimal):not(:has(body.luliy-min-home-page)) {
  background-color: rgb(230,225,212) !important;
  background-image:
    linear-gradient(rgba(230,225,212,0.01), rgba(230,225,212,0.01)),
    url('https://free.picui.cn/free/2026/06/28/6a4119ecea136.png') !important;
  background-size: cover !important;
  background-position: center top !important;
  background-attachment: fixed !important;
  background-repeat: no-repeat !important;
}
html:has(body.luliy-minimal):not(:has(body.luliy-min-home-page)) body.luliy-minimal {
  background: transparent !important;
  background-image: none !important;
}
@media (max-width: 768px) {
  /* 移动端 fixed 背景在部分浏览器有性能/兼容问题，改为随页面滚动 */
  html:has(body.luliy-minimal):not(:has(body.luliy-min-home-page)) {
    background-attachment: scroll !important;
  }
}

/* ════════ 极简模式：隐藏顶部「预计阅读」和文末字数/阅读时长信息条 ════════ */
body.luliy-minimal #luliy-readmeta,
body.luliy-minimal .luliy-readmeta,
body.luliy-minimal #luliy-post-footer-bar {
  display: none !important;
}

/* ── 顶部操作组：[Link] [简洁模式]（沉思者图标右边）── */
#luliy-top-actions {
  position: fixed; top: 14px; z-index: 10002;
  display: inline-flex; align-items: center; gap: 14px;
  height: 44px;
}
#luliy-top-link, #luliy-to-minimal {
  background: transparent; border: none; cursor: pointer;
  font-family: inherit; font-size: 13px; font-weight: 600;
  letter-spacing: 0.5px; color: #fff; padding: 4px 2px;
  opacity: 0.78; transition: opacity 0.2s, transform 0.2s;
  text-decoration: none; white-space: nowrap;
  filter: drop-shadow(0 0 6px rgba(110,199,255,0.5));
}
#luliy-top-link:hover, #luliy-to-minimal:hover { opacity: 1; transform: translateY(-1px); }
#luliy-to-minimal {
  border: 1px solid rgba(255,255,255,0.35); border-radius: 14px;
  padding: 4px 12px;
}
[data-color-mode="light"] #luliy-to-minimal { border-color: rgba(0,0,0,0.25); }
@media (max-width: 600px) {
  #luliy-top-actions { gap: 8px; }
  #luliy-top-link, #luliy-to-minimal { font-size: 11px; }
  #luliy-to-minimal { padding: 3px 8px; }
}

/* ════════════════════════════════════════════════════════════
   极简模式：点击水墨特效
   若干深浅不一的墨滴从点击处向外溅开，由大变小、逐渐淡出。
   纯 CSS 动画，JS 只负责按随机参数生成/销毁 DOM 节点（见 enhance.js
   的 initMinimalInkClick）。
   ════════════════════════════════════════════════════════════ */
.luliy-ink-wrap {
  position: fixed;
  left: 0; top: 0;
...
.luliy-ink-drop {
  ...
  animation: luliyInkDrop 0.7s cubic-bezier(0.16, 0.84, 0.44, 1) forwards;
}
@keyframes luliyInkDrop {
  0%   { opacity: var(--luliy-drop-op, 0.5); transform: translate(0, 0) scale(1.4); }
  100% { opacity: 0; transform: translate(var(--luliy-dx), var(--luliy-dy)) scale(0.15); }
}

/* ════════════════════════════════════════════════════════════
   极简模式：文章 H2~H6 标题下方的毛笔笔触装饰（浅灰、静态常驻，
   跟点击特效无关）。用渐变模拟一笔扫过的笔锋，左淡入、右淡出。
   H1 不加这个装饰。
   ════════════════════════════════════════════════════════════ */
body.luliy-min-article #postBody h2,
body.luliy-min-article #postBody h3,
body.luliy-min-article #postBody h4,
body.luliy-min-article #postBody h5,
body.luliy-min-article #postBody h6 {
  position: relative;
  padding-bottom: 14px;
}
body.luliy-min-article #postBody h2::after,
body.luliy-min-article #postBody h3::after,
body.luliy-min-article #postBody h4::after,
body.luliy-min-article #postBody h5::after,
body.luliy-min-article #postBody h6::after {
  content: '';
  position: absolute;
  left: 0; bottom: 0;
  ...
  background: linear-gradient(90deg,
    ...
    rgba(140,140,140,0.55) 12%,
    rgba(140,140,140,0.32) 45%,
    rgba(140,140,140,0.5) 70%,
    rgba(140,140,140,0) 100%);
}
body.luliy-min-article #postBody h3::after,
body.luliy-min-article #postBody h4::after,
body.luliy-min-article #postBody h5::after,
body.luliy-min-article #postBody h6::after { width: 48px; height: 4px; }
